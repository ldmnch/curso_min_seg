...
Practicaremos con datos georeferenciados en polígonos. Vamos a hacer un análisis de la evolución de homicidios dolosos en la región pampeana del país, así que primero vamos a importar un dataframe con los polígonos provinciales. 
...
```{r}
geo <- geo %>% filter(nomprov %in% c("LA PAMPA", "NEUQUEN", "RIO NEGRO", "CHUBUT", "SANTA CRUZ", "TIERRA DEL FUEGO") )
```
...
#Formato
patagonia <- df %>% filter(provincia %in% c("LA PAMPA", "NEUQUEN", "RIO NEGRO", "CHUBUT", "SANTA CRUZ", "TIERRA DEL FUEGO")) 
```
...
geo <- geo %>% 
        left_join(proyecciones, by = c("nomprov" = "Provincia", "anio"))
```
...
Vemos que si bien Neuquén sigue siendo la provincia con mayor cantidad de casos, Santa Cruz tiene una proporción más alta de casos que la que veíamos en el primer gráfico.
